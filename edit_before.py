...
def parse_args(argv=None):
    ...
    parser.add_argument('--traffic_video', default=None, type=str,
                        help='A path to a video to evaluate on. Passing in a number will use that index webcam.')                    
...
def print_boxes(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    """
    Note: If undo_transform=False then im_h and im_w are allowed to be None.
    """
    if undo_transform:
...
def custom_evalimage(net:Yolact, img):
    frame = torch.from_numpy(img).cuda().float()
    batch = FastBaseTransform()(frame.unsqueeze(0))
    preds = net(batch)

    return print_boxes(preds, frame, None, None, undo_transform=False)
...
class ContourDetection(PipelineProcessor):
    ...
    def __init__(self,net:Yolact, min_contour_width=35, min_contour_height=35, save_image=False, image_dir='images'):
    ...
    def detect_vehicles(self, frame, context):

        matches = []

        # finding external contours
        contours,classes = custom_evalimage(net,frame)
        ...
        for (i, contour) in enumerate(contours):
            ...
            vehicle_type = classes[i]
            matches.append(((x, y, w, h), centroid,vehicle_type))
    ...
    def __call__(self, context):
        ...
        context['objects'] = self.detect_vehicles(frame, context)
        context['fg_mask'] = frame
    ...
    def __call__(self, context):
        ...
        for i, path in enumerate(self.pathes):
            ...
            if (
                ...
                types = [i[2] for i in path]
                vehicle_type = max(set(types), key = types.count)
...
class CsvWriter(PipelineProcessor):
    ...
    def __call__(self, context):
        frame_number = context['frame_number']
        count = _count = np.array(context['vehicle_count'])
...
def traffic_main(net:Yolact,IMAGE_DIR,VIDEO_SOURCE,VIDEO_OUT,PARSE_FRAMES = 15*25,SHAPE = (720, 1280),EXIT_PTS=np.array([
    ...
    pipeline = PipelineRunner(pipeline=[
        ContourDetection(net,save_image=True, image_dir=IMAGE_DIR),
...
def trafficevalvideo(net:Yolact, path:str, out_path:str=None):
    IMAGE_DIR = "./out"
    VIDEO_SOURCE = path
    VIDEO_OUT = out_path
    PARSE_FRAMES = 60*25
    ...
    traffic_main(net,IMAGE_DIR,VIDEO_SOURCE,VIDEO_OUT,PARSE_FRAMES,SHAPE,EXIT_PTS)
##############
...
def evaluate(net:Yolact, dataset, train_mode=False):
    ...
    if args.image is not None:
        ...
        return
    elif args.traffic_video is not None:
        if ':' in args.traffic_video:
            inp, out = args.traffic_video.split(':')
            trafficevalvideo(net, inp, out)
        else:
            trafficevalvideo(net, args.video)
        return
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        if args.image is None and args.video is None and args.images is None and args.traffic_video is None:
